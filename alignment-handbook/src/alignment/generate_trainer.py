...
class Sampler(Trainer):
    ...
    def __init__(
        ...
        def row_in(feature,tokenizer) -> Dict:
            batch = {}
        ...
        for data in train_dataset :
            for key, value in row_in(data,self.tokenizer).items() :
    ...
    def get_dataloader(self,split,for_rank=False) -> DataLoader:
        """
        ...
        if not for_rank :
            if split == 'train' :
                ...
                return super().get_eval_dataloader(eval_dataset=self.eval_dataset)
        ...
        for padded_batch in tqdm(iterable=data_loader, desc="Train dataset reference log probs"):
            ...
            policy_chosen_logp, policy_rejected_logp = self.accelerator.gather_for_metrics(
                (policy_chosen_logp, policy_rejected_logp)
            )
            policy_chosen_logps.append(policy_chosen_logp.cpu())
            policy_rejected_logps.append(policy_rejected_logp.cpu())
        ...
        all_policy_chosen_logps = torch.cat(policy_chosen_logps).float().numpy()
        all_policy_rejected_logps = torch.cat(policy_rejected_logps).float().numpy()
        ...
        if split == 'train' :
            ...
            self.train_dataset = self.train_dataset.add_column(
                name="policy_chosen_logps", column=all_policy_chosen_logps
            )
            self.train_dataset = self.train_dataset.add_column(
                name="policy_rejected_logps", column=all_policy_rejected_logps
            )
    ...
    def tokenize_row_new(self, feature, model: Union[PreTrainedModel, nn.Module] = None) -> Dict:
        print(feature["prompt"])
        return {}
    ...
    def get_batch_samples(self, model, batch: Dict[str, torch.LongTensor], num=None) -> Tuple[str, str]:
        ...
        if num!= None:
            for i in range(num) :
                ...
                output.append(policy_output_decoded)
                #reference_output = pad_to_length(reference_output, self.max_length, self.tokenizer.pad_token_id)
                #reference_output_decoded = self.tokenizer.batch_decode(reference_output, skip_special_tokens=True)

        return output#, reference_output_decoded
    ...
    def get_rank(
        ...
        self.model.eval()
        dataloader = self.get_dataloader(split=split,for_rank=True)
        ...
        file_path = os.path.join(output_dir, save_name+'.json')
        for step, inputs in tqdm.tqdm(enumerate(dataloader)):
            ...
            policy_chosen_logps = inputs["policy_chosen_logps"]
            policy_rejected_logps = inputs["policy_rejected_logps"]
            chosen_rewards = (
                self.beta
                * (
            ...
            sentences_to_add = []
            for prompt, chosen, rejected, chosen_reward, rejected_reward in zip(
                inputs['prompt'], inputs['chosen'], inputs['rejected'], chosen_rewards, rejected_rewards
            ) :
                sentences_to_add.append({prompt: [[chosen,str(chosen_reward)],[rejected,str(rejected_reward)]]})

            #print('policy_output_decoded',policy_output_decoded)
            # 파일에 추가할 문장들의 리스트
            # 파일이 존재하는지 확인하고, 적절하게 처리
                ...
                try:
                    ...
                    with open(file_path, 'r+', encoding='utf-8') as file:
                        ...
                        if not isinstance(data, list):
                            raise ValueError("JSON 파일의 내용이 리스트 형식이 아닙니다.")
                        data.extend(sentences_to_add)
    ...
    def generation(
        ...
        eval_dataloader = self.get_dataloader(split=split)
        start_time = time.time()
    ...
    def evaluation_loop(
        ...
        for step, inputs in tqdm.tqdm(enumerate(dataloader)):
            ...
            policy_output_decodeds = self.get_batch_samples(self.model, inputs, num=5)
            ref_output_decodeds = policy_output_decodeds
            sentences_to_add = []
            prompt_response = defaultdict(list)
            for policy_output_decoded in  policy_output_decodeds :
                for prompt, chosen, pol, ref in zip(
                    inputs['prompt'], inputs['chosen'], policy_output_decoded, policy_output_decoded
                ) :
                    search_term = '[/INST]'
                    search_term_idx = pol.rfind(search_term)
                    prompt_response[prompt].append(pol[search_term_idx + len(search_term):])
            for prompt, responses in prompt_response.items():
                sentences_to_add.append({'prompt': prompt,'response': responses})
            #print('policy_output_decoded',policy_output_decoded)
            # 파일에 추가할 문장들의 리스트
            # 파일이 존재하는지 확인하고, 적절하게 처리
            if not os.path.exists(file_path):
