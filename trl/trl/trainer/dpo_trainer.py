...
class DPOTrainer(Trainer):
    ...
    def __init__(
        ...
        loss_type: Literal["sigmoid", "hinge", "ipo", "kto_pair", "weighted_distill" , "simpo", "weighted"] = "sigmoid",
        ...
        mix=1,
        save_dataset=None,
        save_confidence_name="data.json",
        get_confidence=False
    ):
        self.last_step = 0
        self.threshold = threshold
        self.runnging_confidence = TorchRunningMean( device="cpu")
        self.confidence = torch.tensor([0]) 
        self.confidence_last = torch.tensor([0]) 
        self.gap = 0
        self.alpha = alpha
        self.mix = mix
        self.save_dataset = save_dataset
        self.save_confidence_name = save_confidence_name
        ...
        for data in train_dataset :
            for key, value in self.tokenize_row(data).items() :
                new_dic[key].append(value)
        new_dic |= train_dataset.to_dict()
        train_dataset = new_dic
        train_dataset = Dataset.from_dict(train_dataset)
        if eval_dataset is not None:
            for data in eval_dataset :
                for key, value in self.tokenize_row(data).items() :
                    eval_dic[key].append(value)
            eval_dic |= eval_dataset.to_dict()
            eval_dataset = eval_dic
    ...
    def tokenize_row(self, feature, model: Union[PreTrainedModel, nn.Module] = None) -> Dict:
        ...
        if "ai" in feature.keys() : 
            batch["ai"] = feature["ai"]
                
        if "qi" in feature.keys():
            batch["qi"] = feature["qi"]
    ...
    def dpo_loss(
        ...
        mix_chosen_logps: torch.FloatTensor,
        mix_rejected_logps: torch.FloatTensor,
        reference_free: bool = False,
        a : torch.FloatTensor = None,
        q : torch.FloatTensor = None
        ...
        mix_logratios = mix_chosen_logps - mix_rejected_logps
        if reference_free:
            ref_logratios = 0
        else:
        ...
        mix_logits = mix_logratios - ref_logratios
        ...
        if self.loss_type == "sigmoid":
        ...
        elif self.loss_type == "weighted_distill":
            with torch.no_grad():
                gap = self.confidence.cpu()
                pref_score_chosen = self.beta * mix_logits.detach().cpu() > gap
                #pref_score_chosen = F.sigmoid(self.beta * logits.detach()) >= 0.45
                pref_score_chosen = pref_score_chosen.to(logits.dtype).to(logits.device)
            if self.state.global_step < (self.state.max_steps * 0.2) :
                alpha = 0
            else :
                alpha = self.alpha
                
            if isinstance(a, list):
                a = torch.tensor(a, device=logits.device, dtype=torch.float32)
            if isinstance(q, list):
                q = torch.tensor(q, device=logits.device, dtype=torch.float32)
            losses = torch.zeros_like(logits)
            for ai, qi in zip(a[0], q[0]):
                loss_contrib = (
                    (1 - alpha + alpha *  pref_score_chosen) * ai * (-torch.log((F.sigmoid(self.beta * logits) - qi).clamp(min=1e-6)))
                    + (alpha -  alpha * pref_score_chosen) * ai * (-torch.log((F.sigmoid( - self.beta * logits) - qi).clamp(min=1e-6)))
                )
                losses += loss_contrib
            ...
            losses = torch.zeros_like(logits)
            for ai, qi in zip(a[0], q[0]):
                loss_contrib = ai * (-torch.log((F.sigmoid(self.beta * logits) - qi).clamp(min=1e-6)))
                losses += loss_contrib

        elif self.loss_type == "simpo":
            pi_logratios = policy_chosen_logps - policy_rejected_logps
            pi_logratios = pi_logratios.to(self.accelerator.device)
            logits = pi_logratios - 0.8
            losses = (
                -F.logsigmoid(self.beta * logits) * (1 - self.label_smoothing)
                - F.logsigmoid(-self.beta * logits) * self.label_smoothing
            )
        ...
        mix_margin = self.beta * mix_logits.detach()
    ...
    def get_batch_loss_metrics(
        ...
        confidences = F.sigmoid(chosen_rewards - rejected_rewards).cpu().tolist()
        ...
        metrics[f"{prefix}rewards/mix_margin"] = mix_margin.mean().cpu()
    ...
    def get_batch_loss_metrics_eval(
        self,
        model,
        batch: Dict[str, Union[List, torch.LongTensor]],
        train_eval: Literal["train", "eval"] = "train",
    ):
        """Compute individual rewards for chosen and rejected responses."""
        metrics = {}
        ...
        output_dir = self.args.confidence_dir
        name = self.save_confidence_name
        ...
        if os.path.exists(file_path):
            try:
                ...
                with open(file_path, 'r+', encoding='utf-8') as file:
                    ...
                    for index, c_r, r_r in zip(batch["index"], 
                                            chosen_rewards.cpu().tolist(),
                                            rejected_rewards.cpu().tolist()):
                        self.save_dataset[index] = [c_r, r_r]
                        data[index] = [c_r, r_r]
        ...
        # Calculate loss (needed for trainer)
    ...
    def compute_loss(
        ...
        #with torch.no_grad():
        #    model.eval()
        #    policy_output_decoded, _ = self.get_batch_samples(model, inputs)
        #    model.train()
        #policy_output_decoded = self.tokenize_row(policy_output_decoded)
        #reference_output_decoded = self.tokenize_row(reference_output_decoded)
        ...
        self.store_metrics_step(metrics, train_eval="train")
    ...
    def get_batch_samples(self, model, batch: Dict[str, torch.LongTensor],sample_reference=False) -> Tuple[str, str]:
    ...
    def prediction_step(
        ...
        if not self.get_confidence :
            with torch.no_grad(), prediction_context_manager():
                loss, metrics = self.get_batch_loss_metrics(model, inputs, train_eval="eval")
        else :
            with torch.no_grad(), prediction_context_manager():
                loss, metrics = self.get_batch_loss_metrics_eval(model, inputs, train_eval="eval")
    ...
    def store_metrics_step(self, metrics: Dict[str, float], train_eval: Literal["train", "eval"] = "train") -> None:
        for key, value in metrics.items():
            self._stored_metrics_step[train_eval][key].append(value)

    def save_confidence(self,output_dir: Optional[str] = None , name: Optional[str] = None):
        if output_dir is None:
            output_dir = self.args.output_dir
        if name is None:
            name = "data.json"
        import json
        with open(output_dir + "/"+name, 'w', encoding='utf-8') as json_file:
            json.dump(self.save_dataset, json_file, indent=4)
    ...
    def confidence_update(self):
        if len(self._stored_metrics_step["train"]["rewards/mix_margin"]) > 49 : 
            metrics = {}
            margin = torch.tensor(self._stored_metrics_step["train"]["rewards/mix_margin"])
            margin, _ = torch.sort(margin)
            down = int(50 * self.threshold)
            up = down + 1
            confidence = (margin[down] + margin[up]) / 2
            before_pect = 0
            for i, mar in enumerate(margin) :
                if mar > self.confidence :
                    before_pect = i
                    break   
            before_pect /= 50
            before_pect *= 100
            mean_gap = self.runnging_confidence.before_mean - confidence
            #new_confidence = self.runnging_confidence.update(confidence)
            moving_gap = self.confidence - confidence
            self.confidence = confidence
            metrics[f"rewards/confidence"] = self.confidence.mean().cpu()
            metrics[f"rewards/confidence_mean_diff"] = mean_gap.mean().cpu()
            metrics[f"rewards/confidence_moving_diff"] = moving_gap.mean().cpu()
            metrics[f"rewards/real_percentage"] = before_pect
            self.store_metrics(metrics, train_eval="train")
            #del self._stored_metrics_step["train"]
